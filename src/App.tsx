...
export default function App() {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      try {
        // Filter out and preserve user's template custom presets
        const keysToKeep = Object.keys(localStorage).filter(k => k.startsWith('preset_template_'));
        const keptValues: Record<string, string> = {};
        for (const k of keysToKeep) {
          keptValues[k] = localStorage.getItem(k) || '';
        }
        
        localStorage.clear();
        
        for (const k of keysToKeep) {
          localStorage.setItem(k, keptValues[k]);
        }
      ...
      <header className="h-16 border-b border-slate-200 px-4 lg:px-8 flex items-center justify-between bg-white shrink-0 sticky top-0 z-50 shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
        ...
        <div className="flex items-center gap-2 lg:gap-6">
          ...
          <button 
            ...
            className="flex items-center gap-2 px-2.5 py-1.5 bg-slate-50 hover:bg-red-50 text-slate-600 hover:text-red-600 rounded-xl transition-all border border-slate-200 hover:border-red-100 group"
            title="Sair do Portal"
          >
            <LogOut size={14} className="group-hover:translate-x-0.5 transition-transform" />
            <span className="hidden lg:block text-[10px] font-bold uppercase tracking-wider">Sair</span>
          </button>
        </div>
      </header>
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {Object.values(BASE_TEMPLATES).map((t) => {
